...
```
<img width="2317" height="528" alt="image" src="https://github.com/user-attachments/assets/dff245cb-b9fc-436f-86aa-031f2a223cb7" />

### 3. Перевірка результатів (Фінальний стан)
Для підтвердження коректності всіх операцій було виконано повний перегляд основних таблиць бази даних.

```sql
SELECT * FROM Enrollment;
SELECT * FROM Student;
```
<img width="1658" height="512" alt="image" src="https://github.com/user-attachments/assets/dba420f8-a4da-497d-99d1-59b670c101b1" />

## Висновок
Під час виконання лабораторної роботи №3 було закріплено практичні навички роботи з мовою SQL для маніпулювання даними в межах OLTP-моделі (Online Transaction Processing).

В ході роботи було реалізовано:

- Складні вибірки (SELECT): Опановано механізми об'єднання декількох таблиць за допомогою операторів JOIN. Це дозволило ефективно агрегувати дані з таблиць користувачів, курсів та успішності для отримання цілісної інформації.

- Модифікація даних (INSERT/UPDATE): На практиці реалізовано сценарії реєстрації нових користувачів та оновлення їхніх навчальних показників. Використання вкладених підзапитів дозволило забезпечити логічний зв'язок між таблицями без ручного пошуку ідентифікаторів.

- Забезпечення цілісності: Під час виконання запитів було перевірено роботу обмежень бази даних, зокрема PRIMARY KEY та UNIQUE. Спроби повторного додавання однакових email-адрес підтвердили коректність налаштованих констрейнтів (duplicate key value violates unique constraint).

- Видалення даних (DELETE): Продемонстровано механізм видалення записів. Було враховано особливості каскадного видалення та важливість точного дотримання умов WHERE для збереження консистентності даних.

Робота в середовищі pgAdmin 4 дозволила візуалізувати результати кожної операції, що критично важливо для налагодження бізнес-логіки реальних вебзастосунків. Набуті навички є базовими для подальшої розробки серверної частини (backend) будь-якої інформаційної системи.
